...
RC IX_IndexHandle :: InsertEntryToLeafNodeNoSplit(PageNum noeud, char *key,const RID &rid)
{
	int res; 
	...
	//On teste si notre clé à ajouter est déjà présente dans la feuille
	if(this->EstPresent(noeud, key, ptrAvant))
	{
		//On récupère le numéro du bucket
		int page;
		memcpy(&page, ptrAvant, this->fh.taillePtr);
		this->InsertRIDInBucket(page, rid);
	...
	if(nh.nbCleCrt == 0)
	{
		...
		//On écrit le numéro du bucket
		this->InitBucket(ptrAvant);
		
		//On met le rid dans le bucket
		int bucket;
	...
	else
	{
		...
		//On va retourner en arrière pour accéder au pointeur du bucket
		ptrAvant = ptrApres - this->fh.tailleCle - this->fh.taillePtr;
...
//extrait la clé du milieu d'un noeud,modifie le fils gauche, modifie le fils droit
RC IX_IndexHandle :: ExtractKeyLeaf(const PageNum noeud, char* &key, const PageNum splitNoeud)
{
...
//On se place au pointeur avant la clé
GetPtrInf((nbCleCrt/2)+1, pData);
...
RC IX_IndexHandle :: InsertEntryToLeafNodeSplit(PageNum noeud, char *key,const RID &rid)
{
	int res;
	char *ptrApres;
	char *ptrAvant;
	...
	//La clé n'est pas encore dans les feuilles, il faut donc l'ajouter et initialiser son bucket
	else
	{
			...
			char *val = pData;
			this->ExtractKeyLeaf(noeud,val,filsDroitNum);
			...
			//On remonte au pointeur avant la clé pour y mettre le bucket
			ptrAvant = ptrApres - this->fh.tailleCle - this->fh.taillePtr;
				
			//On initiale un bucket et on écrit le numéro du bucket dans la feuille
			this->InitBucket(ptrAvant);			
			int bucket;
			memcpy(&bucket, ptrAvant, this->fh.taillePtr);
...
RC IX_IndexHandle :: InsertEntryToLeaf(PageNum noeud, char *key,const RID &rid)
{
	int res;
	//On récupère la page 
	PF_PageHandle *page = new PF_PageHandle();
	...
	if(nh.nbCleCrt == this->fh.nbPointeurMax-1)
		this->InsertEntryToLeafNodeSplit(noeud, key,rid);
	//Sinon la feuille n'est pas pleine
	else
		this->InsertEntryToLeafNodeNoSplit(noeud, key,rid);
...
RC IX_IndexHandle :: InsertEntryToNode(PageNum noeud, char *key, int hauteur,const RID &rid)
{
	...
	if(hauteur == this->fh.hauteur)
		this->InsertEntryToLeaf(noeud, key,rid);
	
	//Sinon, On fait une insertion dans un noeud interne
	else
		this->InsertEntryToInt1Node(noeud, key, hauteur,rid);
	
	return 0;
}

//Va chercher le sous arbre dans lequel nous allons insérer notre clé
RC IX_IndexHandle :: InsertEntryToInt1Node(PageNum noeud, char *key, int hauteur,const RID &rid)
{
	int res;
	//On charge la page
	PF_PageHandle *page = new PF_PageHandle();
	...
	for(i = 1; i<=nh.nbCleCrt;i++)
	{
		...
		return res;
			...
			this->InsertEntryToNode(ptr, key, hauteur+1,rid);
		...
		//Si on arrive à la fin du fichier ou si clé = val 
		else if((iVal < iKey && i == nh.nbCleCrt)||iKey == iVal)
		{
...
RC IX_IndexHandle :: InsertEntry(void *pData, const RID &rid)
{
		...
		this->InsertEntryToNode(this->fh.racine, (char*)pData, 1,rid);

	return 0;
}


RC IX_IndexHandle :: InsertRIDInBucket(PageNum bucket, RID rid)
{
	int res;
	//On charge la page
	PF_PageHandle *page = new PF_PageHandle();
	res = this->pf->GetThisPage(bucket, *page);
	if(res != 0)
		return res;
	//On récupère les données de la page
	char *pData;
	...
	ix_BucketHeader bh;
	memcpy(&bh, pData, sizeof(ix_BucketHeader));
	
	//On pointe au bon endroit
		//On passe le header
		pData += sizeof(ix_BucketHeader);
		//On passe tout les RID
		pData += bh.nbRidCrt;
		
	//On écrit le RID
	memcpy(pData, &rid, sizeof(RID));	
	
	//On augment le nb courant de RID dans le bucket
	bh.nbRidCrt ++;
	...
	memcpy(pData, &bh, sizeof(ix_BucketHeader));
	
	//On force l'écriture et on upin la page
	res = this->pf->ForcePages(bucket);
		if(res != 0)
		return res;
	res = this->pf->UnpinPage(bucket);
		if(res != 0)
		return res;
...
bool IX_IndexHandle :: EstPresent(PageNum leaf, char *key,char *&bucket)
{
	int res;
	//On charge la page
	PF_PageHandle *page = new PF_PageHandle();
	res = this->pf->GetThisPage(leaf,*page);
	if(res != 0)
		return res;
	//On charge les données
	char *pData;
	...
	memcpy(&ikey, key, this->fh.tailleCle);
	int ival;
	//On va testers chaque clé du noeud
	for(i = 1 ; i<=nh.nbCleCrt; i++)
	{
		//On récupère la clé à l'indice i
		GetCle(i,pData);
		memcpy(&ival, pData, this->fh.tailleCle);
		...
		if(ikey == ival)
		{
			...
			//On repointe au debut du fichier
			res = page->GetData(pData);
			if(res != 0)
				return res;
			//On récupère le pointeur avant ival
			GetPtrInf(i,pData);	
			bucket = pData;
			
			//On unpin la page
			res = this->pf->UnpinPage(leaf);	
			if(res != 0)
			return res;
			
			return true;
		}
		//On retourne au début du pointeur
		res = page->GetData(pData);
		if(res != 0)
			return res;	
...
RC IX_IndexHandle :: InitBucket(char *&ptr)
{
	int res;
	//On alloue une nouvelle page
	PF_PageHandle *page = new PF_PageHandle();
	res = this->pf->AllocatePage(*page);
	if(res != 0)
		return res;
		
	//On récupère le numéro de la page
	PageNum numero;
	page->GetPageNum(numero);	
	
	//On fabrique un bucket header
	ix_BucketHeader bh;
	bh.nbRidCrt = 0;
	...
	//On écrit le header dans la page
	memcpy(pData, &bh, sizeof(ix_BucketHeader));
	
	//On écrit le numéro du bucket dans ptr
	memcpy(ptr, &numero, this->fh.taillePtr);
	
	//On force l'écriture et on unpin la page
	res = this->pf->ForcePages(numero);
	if(res != 0)
		return res;	
	res = this->pf->UnpinPage(numero);
	if(res != 0)
		return res;	
...
//Force l'écriture en disque de toutes les pages de l'index
RC IX_IndexHandle :: ForcePages ()
{
	int res;
	res = this->pf->ForcePages();
